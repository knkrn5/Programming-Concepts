# **Network Concepts**

> **what is net: -** net is just the transfering/receiving of data, whether u transfer/download data in within ur devices using wire or wirelessly, or whether u transfer(upload)/receive(download) data from any long sea cable connected device called cloud, net is just a simple data transferring and receiving

---

## **Computer Networking**

- **[🔗Type of network](./type-of-network/ton.svg)**
- **[🔗Interfaces](./interfaces/interfaces.svg)**
  - **[🔗MAC address](./media-access-control/mac.svg)**
    - **[🔗Address Resolution Protocol](./Address-Resolution-Protocol/arp.svg)**
    - **[🔗Network address translation](./network-address-translation/NAT.svg)**
- **[🔗Domain Name System](./domain-name-system/dns.svg)**
- **[🔗Network address translation](./network-address-translation/NAT.svg)**

<!-- Protocols -->

### **Protocols: -** A set of rules

- **Transmission Control Protocol**
  - **[🔗TCP Segments](./protocols/transmission-control-protocol/TCP-segment/tcps.svg)**
    - **[🔗TCP Handshake](./protocols/transmission-control-protocol/tcp-handshake/tcph.svg)**
- **user Datagram Protocol**
  - **[🔗UDP Datagrams](./protocols/user-datagram-protocol/UDP-datagram/udpd.svg)**
    - **[🔗UDP Exchange](./protocols/user-datagram-protocol/udp-exchange/udpe.svg)**
- **Internet Control Message Protocol(ICMP)**
  - **[🔗ICMP](./protocols/internet-control-message-protocol/icmpv6/icmpv6.svg)**
- **Internet Protocol**
  - **[🔗IP addresses](./protocols/internet-protocol/ip.svg)**
    - **[🔗IPv4 packets](./protocols/internet-protocol/ip-packet/ipv4-packet/ipv4p.svg)**
      - **[🔗Address Resolution Protocol](./Address-Resolution-Protocol/arp.)**
    - **[🔗IPv6 packets](./protocols/internet-protocol/ip-packet/ipv6-packet/ipv6p.svg)**
      - **[🔗Network address translation](./network-address-translation/NAT.svg)**

- **Secure Shell**
  - **[🔗SSH](./protocols/secure-shell/ssh.svg)**
    - **[🔗SSH Handshake](./protocols/secure-shell/ssh-handshake/sshh.svg)**

- **HyperText transfer protocol(HTTP)**
  - **[🔗HTTP request](./protocols/hypertext-transfer-protocol/http-req/httpreq.svg)**
  - **[🔗HTTP response](./protocols/hypertext-transfer-protocol/http-res/httpres.svg)**
  - **[🔗HTTP](./protocols/hypertext-transfer-protocol/http.svg)**

### **Network Frames**

- **[🔗Ethernet Frame](./network-frames/ethernet-frame/ef.svg)**
- [🔗Wifi Frame] = upcoming

<!-- Network Models -->

### **Network Models**

- **[🔗OSI Model](./network-models/osi-model/osi.svg)**
  - For more details on OSI Model, visit [visit](./network-models/osi-model/README.md)

- **[🔗TCP/IP Model](./network-models/tcp-ip-model/tcpip.svg)**

## **Other Networking Concepts**

- **[🔗networking terms](./networking-terms/networking-terms.svg)**
- **[🔗casts](./networking-terms/cast/cast.svg)**
- **[🔗ports](./networking-terms/ports/ports.svg)**
- **[🔗 Network Topologies](./network-topologies/nt.svg)**

## **VPN**

1. upcoming

## **Proxy Server**

1. upcoming
